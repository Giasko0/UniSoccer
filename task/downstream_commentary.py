...
def train(args):
    ...
    # HERE, you can add other datasets by your self (e.g. SoccerReplay-1988)
    train_dataset = dataset_type(json_file="./train_data/json/MatchTime/classification_train.json",
                       video_base_dir="FOLDER_OF_VIDEO_CLIPS_OF_MATCHTIME",
                       npy_dir=args.npy_dir)
    val_dataset = dataset_type(json_file="./train_data/json/MatchTime/classification_valid.json",
                       video_base_dir="FOLDER_OF_VIDEO_CLIPS_OF_MATCHTIME",
                       npy_dir=args.npy_dir)
    torch.cuda.init()
    ...
    model = commentary_model_type(device=args.device, num_features=args.num_features, need_temporal=args.need_temporal)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--npy_dir", type=str, default="SIGLIP_FEATURES") # Here you can use any feature (you can reference them on (https://www.soccer-net.org/data), before using them, you have to transform the directory format as features of video clips.
